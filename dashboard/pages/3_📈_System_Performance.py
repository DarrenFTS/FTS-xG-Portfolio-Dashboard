...
with cl2:
    ...
    fig.update_layout(height=310,template='plotly_white',margin=dict(l=0,r=0,t=5,b=50),
        legend=dict(orientation='h',y=-0.3,font=dict(size=10)),yaxis_title="P/L (pts)")
...
with cr2:
    ...
    fdd.update_layout(height=310,template='plotly_white',margin=dict(l=0,r=0,t=5,b=50),yaxis_title="Drawdown (pts)")
...
with ct:
    st.dataframe(lg.style.format({'PL':'{:+.2f}','ROI%':'{:+.2f}%','SR%':'{:.2f}%','AvgOdds':'{:.2f}','MaxDD':'{:.2f}'})
                   .applymap(rc,subset=['ROI%']),use_container_width=True,hide_index=True)
with cb:
...
def hc(v):
    ...
    return 'background-color:#FDE8E8;color:#9B1C1C'
st.dataframe(pv.style.format('{:+.2f}%',na_rep='—').applymap(hc),use_container_width=True)

st.divider()
st.subheader("xG Distribution")
lbl="6G FH xG" if sc=="FHG Lay U0.5" else "6G Match xG"
fx=px.histogram(sb,x='xg_value',nbins=40,color='system',color_discrete_map=MKT,labels={'xg_value':lbl})
fx.update_layout(height=270,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),showlegend=False)
