...
with c1:
    fo=px.bar(og,x='ob',y='ROI%',color='system',barmode='group',
              category_orders={'ob':OO},color_discrete_map=MKT,labels={'ob':'Odds Band','ROI%':'ROI %'})
    fo.update_layout(height=330,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
...
fm.update_layout(height=300,template='plotly_white',margin=dict(l=0,r=0,t=10,b=60),
    xaxis_tickangle=-45,yaxis_title="P/L (pts)")
st.plotly_chart(fm,use_container_width=True)

st.divider()
st.subheader("Strike Rate by System")
c3,c4=st.columns(2)
with c3:
    fh=px.histogram(b2,x='pl',color='system',nbins=50,color_discrete_map=MKT,labels={'pl':'P/L per bet'})
    fh.update_layout(height=280,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
        legend=dict(orientation='h',y=-0.25))
...
with c4:
    ...
    fsr.update_layout(height=280,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
...
fr.update_layout(height=280,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),yaxis_title="Rolling ROI %")
