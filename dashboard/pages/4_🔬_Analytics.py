...
with c1:
    ...
    fo.update_layout(height=340,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
...
with c2:
    ...
    st.dataframe(ov.style.format({'PL':'{:+.2f}','ROI%':'{:+.2f}%'}),use_container_width=True)
...
mn['ROI%']=mn['PL']/mn['Bets']*100; mn['clr']=mn['PL'].apply(lambda x:'#217346' if x>0 else '#9B1C1C')
fm=go.Figure(go.Bar(x=mn['ym'],y=mn['PL'].round(2),marker_color=mn['clr'],
    text=[f"{p:+.2f}" for p in mn['PL']],textposition='outside',
    hovertemplate='%{x}<br>P/L: %{y:+.2f}<extra></extra>'))
fm.update_layout(height=310,template='plotly_white',margin=dict(l=0,r=0,t=10,b=60),
...
c3,c4=st.columns(2)
with c3:
    fh=px.histogram(b2,x='pl',color='system',nbins=50,color_discrete_map=MKT,
                    labels={'pl':'P/L per bet'})
    fh.update_layout(height=290,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
        legend=dict(orientation='h',y=-0.22))
...
with c4:
    ...
    fsr.update_layout(height=290,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
...
rs_data=b2[b2['system']==rs].sort_values('date').copy()
rs_data['rr']=rs_data['pl'].rolling(50,min_periods=20).mean()*100
fr=go.Figure(go.Scatter(x=rs_data['date'],y=rs_data['rr'].round(2),mode='lines',
...
fr.update_layout(height=290,template='plotly_white',margin=dict(l=0,r=0,t=10,b=10),
...
fs=b2.groupby(['system','league']).agg(Bets=('pl','count'),PL=('pl','sum'),
    Won=('won','sum'),AvgOdds=('odds','mean')).reset_index()
fs['ROI%']=fs['PL']/fs['Bets']*100; fs['SR%']=fs['Won']/fs['Bets']*100
fs=fs.rename(columns={'system':'System','league':'League'})
fs=fs[['System','League','Bets','PL','ROI%','SR%','AvgOdds']].sort_values(['System','ROI%'],ascending=[True,False])
st.dataframe(fs.style.format({'PL':'{:+.2f}','ROI%':'{:+.2f}%','SR%':'{:.2f}%','AvgOdds':'{:.2f}'}),
